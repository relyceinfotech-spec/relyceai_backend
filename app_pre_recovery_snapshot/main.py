...
def _extract_key_points(text: str, limit: int = 6) -> list[str]:
    points = []
    ...
    for pattern in patterns:
        for match in re.finditer(pattern, text or ""):
            ...
            if url in seen:
                continue
            seen.add(url)
            found.append({"name": name, "url": url, "trust": None})
            if len(found) >= limit:
                return found
    return found
...
def _infer_answer_type(chat_mode: str) -> str:
    mode = (chat_mode or "normal").lower()
    if mode == "normal":
        return "plain"
    if mode == "business":
        return "analysis"
...
def _build_response_blocks(answer: str, key_points: list[str]) -> list[dict]:
    blocks = []
    if answer:
...
def _normalize_chat_payload(result: dict, chat_mode: str) -> dict:
    response_text = str((result or {}).get("response") or "").strip()
    key_points = _extract_key_points(response_text)
    sources = _extract_sources(response_text)
    mode_used = str((result or {}).get("mode_used") or chat_mode or "normal")
    tools_activated = list((result or {}).get("tools_activated") or [])
    answer_type = _infer_answer_type(chat_mode)

    if chat_mode == "normal":
        confidence = 0.92
    elif chat_mode == "business":
        confidence = 0.82
    elif chat_mode in {"agent", "deepsearch"}:
        confidence = 0.78
    else:
        confidence = 0.75
    ...
    structured_payload = {
        ...
        "blocks": _build_response_blocks(response_text, key_points),
    }
    ...
    return {
        **dict(result or {}),
        ...
        "confidence_level": _confidence_level(confidence),
        "answer_type": answer_type,
        "metadata": structured_payload["metadata"],
        "blocks": structured_payload["blocks"],
...
async def chat(request: ChatRequest, req: Request, user_info: dict = Depends(get_current_user)):
    ...
    try:
        ...
        final_payload = _normalize_chat_payload(result, request.chat_mode)
    ...
    async def generate():
        ...
        try:
            ...
            try:
                ...
                final_payload = _normalize_chat_payload(
                    {
                        "success": True,
                ...
                yield f"data: {json.dumps({'type': 'final_answer', 'content': final_payload['structured_response']})}\n\n"
                yield f"data: {json.dumps({'type': 'done', 'content': ''})}\n\n"
